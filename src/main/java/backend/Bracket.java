    ...
    * EXECUTE NEEW PLAN IN NOTEBOOOK!!!!!
    ...
    public void createPools(int poolSize) {
        ...
        for (int i = 1; i < poolCount + 1; i++) {
            orderedPoolMap.put(i, new Pool("Pool " + i, this, 1, poolQueries));
    ...
    private void createWinnersSideMatches(Pool pool) {
        ...
        for (int i = 0; i < winnersRounds; i++) {
            while (matchPositionMarker % 100 <= maxMatchesInRound) {
                Match newMatch = new Match(matchPositionMarker, pool, "winners", this, matchQueries);
...
//            totalLosersRoundsCount++; // need to increment to account for the first losers round being filled.
        ...
        for (int i = 0; i < losersRounds + 1; i++) {
            while (matchPositionMarker % 100 <= maxMatchesInRound) {
                Match newMatch = new Match(matchPositionMarker, pool, "losers", this, matchQueries);
    ...
    private int[] generatePoolOrder(int poolCount) {
        ...
        for (int seed = 1; seed <= poolCount / 2; seed++) {
            int previousPosition = prev[seed - 1];
            result[seed - 1] = 2 * previousPosition - 1;
            result[poolCount - seed] = 2 * previousPosition;
        }
        return result;
    }
    ...
    public void mergePools(NavigableMap<Integer, Pool> activePools) {
        ...
        Pool newPool = new Pool("Pool " + totalBracketPoolCountMark, this, currentPoolStageMarker, poolQueries);
        ...
        for (Pool pool : activePools.values()) {
            ...
            if (poolMatchMaxCount >= 8) {
                ...
                poolMatchMaxCount = 1;
                newPoolKey++;
                newPool = new Pool("Pool " + totalBracketPoolCountMark, this, currentPoolStageMarker, poolQueries);
            ...
            if (!playerParity) {
                Match newWinnersMatch = new Match(poolWinner, subRoundMarker, 1, newPool, "winners", this, matchQueries);
                newPool.addWinnersMatch(newWinnersMatch);
                Match newLosersMatch = new Match(poolLoser, subRoundMarker, 1, newPool, "losers", this, matchQueries);
...
//                poolLoser.updateBracketMatchHistory(existingLosersMatch.getMatchGame(), existingLosersMatch);
    ...
    public void finalizeAllPlacements() {
        for (Player player : orderedPlayerMap.keySet()) {
            if (!player.isPlacementFinalized()) {
                ...
                player.updatePlayerTotalMatchHistory(parentTournament.getTournamentID(), gameName);
            }
        }
    }


    public Pool findPool(Integer stage, Integer poolNumber) { return bracketStagesMap.get(stage).get(poolNumber); }
    ...
    public Player getBracketWinner() { return bracketWinner; }

    public Long getGetBracketID() {return bracketID; }
    ...
    public int getBracketTier() { return bracketTier; }
